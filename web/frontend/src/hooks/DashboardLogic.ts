...
export function useDashboardLogic() {
  ...
  // ================== Computed Values ==================
  const totalStock = useMemo(() => products.reduce((acc, p) => acc + (p.inventory.total_stock || 0), 0), [products]);
  const totalIn = useMemo(() => products.reduce((acc, p) => acc + (p.inventory.stock_in || 0), 0), [products]);
  const totalOut = useMemo(() => products.reduce((acc, p) => acc + (p.inventory.stock_out || 0), 0), [products]);
